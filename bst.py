...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            larger keys.
        """
        # If current node has no children, return None
        if current is None:
            return current
        # If there's a child on the right node, find the minimum the smallest node greater than current and return it
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        """
        # if the left node of the current node has a child, find the lowest (left-most) node of the BST.
